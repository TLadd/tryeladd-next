...
Nothing in the above examples is particularly compelling for the code-first approach. Indeed, the first time I looked at Nexus, I thought it was a step backwards to something like vanilla graphql-js compared to the approach popularized by Apollo where you get to write the nice SDL. Indeed, I still think the biggest benefit of the schema-first approach is that writing and reading the SDL is easier than the Nexus API. `appearsIn: [Episode!]!` is a lot easier to parse than `t.field("appearsIn", { type: nonNull(list(nonNull(Episode))) })`.

On the reading front, the code-first approach at least generates a schema file that can be easily referred to and used in other tooling, but I definitely find it's easier to initially write object types using the SDL.
...
Both schema-first and code-first approaches work pretty well for developing a GraphQL server.  